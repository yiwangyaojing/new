...
class TeamService extends Service {
      ...
      return ctx.model.XTeam.create(team, { transaction: t }).then(function(result0) { // 保存team
        ...
        if( !(String(agents) == 'null' || String(agents) == 'undefined') ){
          for (let agent of agents) {
            ...
            ctx.model.XUsers.update({ company_id: req.company_id, company_name: req.company_name }, { where: { openid: agent.open_id } });
          }
  ...
  async validatePower(team, open_id) {
    ...
    if (teamLevel === FileType.TeamLevel.company) { // 公司级别团队
      ...
      // 查询操作人所在的最高团队等级
      const teamUser = await this.ctx.model.XTeamUser.findOne({ where: { open_id, user_rank: FileType.UserRank.admin },
