...
<template>
  <div class="flex-v full">
    <el-row class="breadcrumb warp" type="flex" align="middle">
      <el-col>
        <h2>用户管理</h2>
        <p>管理用户数据，用户归属机构，每个用户可以分配不同的角色。</p>
      </el-col>
      ...
      <el-col :span="18">
        <div class="main">
          <el-row type="flex" justify="start" align="middle" :gutter="8" class="warp">
            ...
            <el-col><el-button size="small" type="primary" icon="el-icon-circle-plus-outline" @click="createClicker">添加用户</el-button></el-col>
            <el-col>
            ...
            <el-table size="small" :data="data.content" border :max-height="500" style="width: 100%">
              ...
              <el-table-column prop="roles" label="拥有角色">
                <template slot-scope="scope">
                  <el-tag type="danger" size="medium" v-for="(item, index) in scope.row.roles" :key="index">{{item}}</el-tag>
                </template>
              </el-table-column>
              <el-table-column fixed="right" label="操作" align="center" width="100">
                <template slot-scope="scope">
                  <el-tooltip class="item" effect="dark" content="角色分配" placement="bottom">
    ...
    <!-- 新增 -->
    <el-dialog title="用户添加/编辑" :visible.sync="views.form.show" width="540px">
      <el-form :model="views.form.data" :rules="views.form.rules" ref="views.form.data" label-width="140px">
        ...
        <el-form-item label="用户名" prop="username" >
          <el-input v-model="views.form.data.username" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="用户密码" prop="password">
          <el-input v-model="views.form.data.password" auto-complete="off"></el-input>
        </el-form-item>
    ...
    <el-dialog title="绑定角色" :visible.sync="views.bindForm.show" width="600px">
      <el-row class="warp" type="flex" justify="center">
        <el-col>
          <el-transfer
            ...
            :props="views.transfer.props"
            v-model="views.transfer.bound"
            @change="handleChangeBind"
            :data="views.transfer.data">
          </el-transfer>
        </el-col>
      </el-row>
      <div slot="footer" class="dialog-footer">
        <el-button @click="views.bindForm.show = false">关闭</el-button>
        <!--<el-button type="primary" @click="handleSubmitBind()">确 定</el-button>-->
      </div>
    </el-dialog>

    <!--<el-dialog title="用户授权" :visible.sync="views.bindForm.show" width="40%">
      <el-tree
        ref="tree"
...
export default {
  ...
  methods: {
    ...
    loadUsers: function (page, orgCode) {
      let pageable = {}
      pageable.page = page
      pageable.size = this.data.page.size
      this.views.loading = true

      this.$axios.get('/organizations/' + orgCode + '/users', null, {params: pageable}).then(response => {
    ...
    bindClicker: function (row) {
      ...
      this.views.bindForm.user = row.code
      // 查询所有的可配角色
      this.$axios.get('/organizations/' + this.$refs.tree.getCurrentKey() + '/roles/list').then(response => {
        console.log('bindClicker.response.body: %o', response.body)
        this.views.transfer.data = response.body
      })
      // 查询当前用户的已有角色
      this.$axios.get('/users/' + row.code + '/roles').then(response => {
        console.log('bindClicker.response.body: %o', response.body)
        this.views.transfer.bound = response.body
      })
    },
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (this.views.form.data.id) {
            this.$axios.put('/users/' + this.views.form.data.id, this.views.form.data).then(response => {
            ...
            this.$axios.post('/users', this.views.form.data).then(response => {
              this.views.form.show = false
              this.$message.success('保存成功')
              this.init()
            }).catch((error) => {
    ...
    handleBatchDelete: function () {
      if (!this.$refs.tree.getCurrentKey()) {
        ...
        this.views.form.data = {}
        this.$axios.delete('/users/' + this.$refs.tree.getCurrentKey()).then(response => {
    ...
    handleChangeBind: function (value, mode, keys) {
      console.log('handleChangeBind > user: %s, keys: %o', this.views.bindForm.user, keys)
      if (mode === 'left') {
        // 移除
        this.$axios.post('/users/' + this.views.bindForm.user + '/roles/remove', keys).then(response => {
          this.loadUsers(0, this.$refs.tree.getCurrentKey())
        }).catch(error => {
          this.$message.error(error.message)
        })
      } else {
        // 授权
        this.$axios.put('/users/' + this.views.bindForm.user + '/roles', keys).then(response => {
          this.loadUsers(0, this.$refs.tree.getCurrentKey())
        }).catch(error => {
          this.$message.error(error.message)
        })
